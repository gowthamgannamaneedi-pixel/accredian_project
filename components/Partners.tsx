...
/* ─── Brand text logos — styled text in exact brand colors ───────────── */
// Used for companies where SVG paths are unavailable or don't look correct
interface BrandText { label: string; color: string; size?: string }

/* ─── Company list ────────────────────────────────────────────────────── */
type CompanyEntry =
  | { name: string; type: 'si';    icon: { path: string; hex: string } }
  | { name: string; type: 'brand'; brand: BrandText }
  | { name: string; type: 'text';  textColor: string; bg: string };

const COMPANIES: CompanyEntry[] = [
  // simple-icons — verified keys, always renders
  ...
  // Brand text logos — bold styled wordmarks in exact brand colors
  { name: 'Google',        type: 'brand', brand: { label: 'Google',    color: '#4285F4', size: 'text-base' } },
  { name: 'Microsoft',     type: 'brand', brand: { label: 'Microsoft', color: '#00A4EF', size: 'text-sm'  } },
  { name: 'Amazon',        type: 'brand', brand: { label: 'amazon',    color: '#FF9900', size: 'text-base' } },
  { name: 'IBM',           type: 'brand', brand: { label: 'IBM',       color: '#1F70C1', size: 'text-xl'  } },
  { name: 'Oracle',        type: 'brand', brand: { label: 'ORACLE',    color: '#F80000', size: 'text-sm'  } },
  { name: 'Adobe',         type: 'brand', brand: { label: 'Adobe',     color: '#FF0000', size: 'text-base' } },
  // Styled text badges
...
function CompanyLogo({ entry }: { entry: CompanyEntry }) {
  if (entry.type === 'si') {
    return (
      <div className="flex flex-col items-center gap-2">
        <svg viewBox="0 0 24 24" width={28} height={28} aria-label={entry.name} fill={`#${entry.icon.hex}`}>
          <path d={entry.icon.path} />
        </svg>
        <span className="text-[10px] font-bold text-slate-500 tracking-wide">{entry.name}</span>
      </div>
    );
  }
  ...
  if (entry.type === 'brand') {
    return (
      <span
        className={`font-extrabold tracking-tight leading-none ${entry.brand.size ?? 'text-base'}`}
        style={{ color: entry.brand.color }}
      >
        {entry.brand.label}
      </span>
    );
  ...
  return (
    <span
      ...
      style={{ color: entry.textColor, background: entry.bg, borderColor: `${entry.textColor}33` }}
    >
      {entry.name}
    </span>
  );
